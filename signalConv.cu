...
__global__ void FIR(float* signal, size_t signalLength, size_t filterLength, 
    ...
    __shared__ float sharedSignal[1024 + 32];

    int threadIx = threadIdx.x;
    if (pos < signalLength) {
        sharedSignal[threadIdx.x] = signal[pos];
    }
    if (threadIdx.x < WARPSIZE && pos + blockDim.x < signalLength) {
        sharedSignal[threadIdx.x + blockDim.x] = signal[pos + blockDim.x];
    }
    __syncthreads();

    #pragma unroll
    for (int i = 0; i < 32; ++i) {
        if (pos + i < signalLength) {
            
            float summand = sharedSignal[threadIdx.x + i] * FILTER[laneId];
            float res = summand;
...
int main() {
    ...
    FIR<<<blocks, threads>>>(signalDevice, signalLength, filterLength, outDevice);
    cudaMemcpy(outSignal.data(), outDevice,
        outSignalLength * sizeof(float), cudaMemcpyDeviceToHost);
    std::cout << "Output: ";
    for (auto elem : outSignal) {
        std::cout << (int)elem << " ";
    }
